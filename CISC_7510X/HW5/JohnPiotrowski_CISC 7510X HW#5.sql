...
min_max_tmonth_per_symbol as (
    select 
		symbol, 
        min(extract(month from tdate)) as first_trade_month,
        max(extract(month from tdate)) as last_trade_month
    from valid_2013_stocks
    group by symbol
	having min(extract(month from tdate)) = 1  -- Keep only stocks that started in Jan
		and max(extract(month from tdate)) = 12  -- Keep only stocks that traded until Dec
),
...
stock_pairs_2013 as (
	...
	where CONCAT(s1.symbol,':',s2.symbol) like '%MSFT:GOOG%'
),
